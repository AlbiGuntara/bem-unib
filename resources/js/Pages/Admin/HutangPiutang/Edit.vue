...
//    FORM (ISI DARI DATABASE)
const form = useForm({
    jenis: props.item.jenis,
    tanggal: props.item.tanggal ? props.item.tanggal.substring(0, 10) : "",
    nama: props.item.nama,
    foto: null,
    kontak: props.item.kontak,
    nominal: props.item.nominal,
    keterangan: props.item.keterangan,
    jatuh_tempo: props.item.jatuh_tempo
        ? props.item.jatuh_tempo.substring(0, 10)
        : "",
...
onMounted(() => {
    if (props.item.foto) {
        previewFoto.value = `/storage/${props.item.foto}`;
    }

    if (props.item.bukti_transaksi) {
        previewBukti.value = `/storage/${props.item.bukti_transaksi}`;
    }
});
...
function handleFileChange(e, field) {
    ...
    if (file.type.startsWith("image/")) {
        if (field === "foto") {
            ...
            previewBukti.value = URL.createObjectURL(file);
        }
    }
}
...
function submit() {
    form.transform((data) => ({
        ...data,
        _method: "put",
    })).post(route("hutang-piutang.update", props.item.id), {
        forceFormData: true,
        preserveScroll: true,
        onError: (errors) => {
